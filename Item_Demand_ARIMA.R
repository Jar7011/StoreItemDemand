...
# Create ARIMA recipe
recipe_1 <- recipe(sales ~ ., data = train_combination_1) %>% 
  step_rm(c('store', 'item'))
recipe_2 <- recipe(sales ~ ., data = train_combination_2) %>% 
  step_rm(c('store', 'item'))
...
# Tune the models
cv_results_1 <- modeltime_calibrate(arima_wf_1, new_data = testing(cv_split_1))
cv_results_2 <- modeltime_calibrate(arima_wf_2, new_data = testing(cv_split_2))
